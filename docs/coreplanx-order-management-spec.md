# CorePlanX – Auftragsmanagement: Produktspezifikation
...
Client-seitig wird `API_CONFIG` (siehe `src/app/core/config/api-config.ts`) fuer Basis-URL genutzt; ueberschreibbar via Meta-Tag `coreplanx-api-base` oder `window.__COREPLANX_API_BASE__`.
