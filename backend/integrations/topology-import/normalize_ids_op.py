...
def build_pattern(prefix: str, fill: str) -> re.Pattern:
    ...
    # If fill is empty, we just match the prefix once and capture the rest
    if fill == "":
...
def main():
    parser = argparse.ArgumentParser(description="Normalize IDs by stripping a leading PREFIX and repeated FILL characters. Example: DE00AB -> AB")
    parser.add_argument('--in', dest='infile', required=True, help='Input OP CSV path')
    ...
    parser.add_argument('--columns', nargs='+', default=['ID'], help='Column(s) to normalize (default: ID)')
